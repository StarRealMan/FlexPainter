...
def chunk_batch(func, chunk_size: int, *args, **kwargs):
    if chunk_size <= 0:
...
def get_plucker_rays(rays_o, rays_d):
...
def polar_to_c2w(elevation: float, azimuth: float, distance: float):
    """
    Compute L = p - C.
    Normalize L.
